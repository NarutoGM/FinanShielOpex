...
import { FileText, Users, LogOut, X, ChevronRight, Tag } from "lucide-react";
...
export default function Sidebar({
  ...
  // Estado para trackear la vista actual desde query params
  const [currentView, setCurrentView] = useState<string | null>(null);

  // Efecto para leer los query params
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        <div className="h-24 flex items-center justify-center px-8 border-b border-stone-100">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="ml-auto lg:hidden text-stone-400 hover:text-stone-900"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
          ...
          {MENU_ITEMS.map((item) => {
            const isConceptos = item.path.includes("view=conceptos");
            const isActive = isConceptos
              ? currentView === "conceptos"
              : pathname === item.path && !currentView;
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <ChevronRight
                    size={16}
                    className="text-stone-900 opacity-50"
                  />
                )}
              </Link>
            );
